...
export default function FinalGameCard({ game, onOpen }: Props) {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/[0.08] px-6 py-4 sm:px-7">
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-sm font-medium text-[#94a3b8]">FINAL</span>
          {game.game_date && (
            <span className="text-sm text-[#94a3b8]">· {formatGameDate(game.game_date)}</span>
          )}
        </div>
        {outcome.kind === 'winner' && (
          <span className="rounded-full bg-[#facc15]/15 px-3.5 py-1.5 text-sm font-semibold text-[#facc15]">
            {outcome.teamName} wins
          </span>
        )}
        {outcome.kind === 'tie' && (
          <span className="rounded-full bg-white/10 px-3.5 py-1.5 text-sm font-semibold text-[#94a3b8]">
            Tied
          </span>
        )}
      </div>
      ...
      <div className="px-6 py-5 sm:px-7 sm:py-6">
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col items-start">
            <span
              className={`font-heading text-3xl font-bold ${awayWon ? 'text-[#facc15]' : 'text-white'}`}
            >
              {game.away_team}
              {awayWon && (
                <span className="ml-2 text-base font-semibold text-[#facc15]/90">W</span>
              )}
            </span>
            <span
              className={`font-heading text-5xl font-bold tabular-nums leading-tight ${awayWon ? 'text-[#facc15]' : 'text-white'}`}
            >
              {game.away_score}
            </span>
          </div>
          ...
          <div className="flex flex-col items-end">
            <span
              className={`font-heading text-3xl font-bold ${homeWon ? 'text-[#facc15]' : 'text-white'}`}
            >
              {game.home_team}
              {homeWon && (
                <span className="ml-2 text-base font-semibold text-[#facc15]/90">W</span>
            ...
            <span
              className={`font-heading text-5xl font-bold tabular-nums leading-tight ${homeWon ? 'text-[#facc15]' : 'text-white'}`}
            >
              {game.home_score}
            </span>
          </div>
